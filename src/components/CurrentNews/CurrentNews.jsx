...
let CurrentNews = (props) => {
    return (<Card className={classes.mainWrapper}>
        <Card.Header className={classes.commentTitle}>
            {props.news.score} <RaitingImg className={classes.raitingImg}/> by <span className={classes.newsCreatorName}>{props.news.by}</span> {transformContentDate(props.news.time)}
        </Card.Header>
